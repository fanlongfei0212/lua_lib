...
--入参:
    ...
    --switch_db为布尔，true是需要进行数据库切换，false为不切换
    --db为指定的数据库索引，值为number，在switch_db为true的情况下，如果db为nil则根据key值进行数据库索引的自动计算
local function switch_db_exec_command(key, commands, is_return, switch_db, db)
    if type(db) ~= "number" or not check_db(db) then
...
--返回:无返回
function _M.set(set_command, switch_db, db)
    if set_command and type(set_command) == "table" and next(set_command) and not common_util.is_array(set_command) and set_command.key and set_command.value then
        if switch_db and type(switch_db) == "boolean" then
            local pipeline_commands = {{"set", set_command.key, set_command.value}}
            switch_db_exec_command(set_command.key, pipeline_commands, false, switch_db, db)
        end
    end
end
...
--入参:key
--返回:查询结果(字符串)
function _M.get(key, switch_db, db)
    local result = nil
    if key and type(key) == "string" then
        if switch_db and type(switch_db) == "boolean" then
            local pipeline_commands = {{"get", key}}
            result = switch_db_exec_command(key, pipeline_commands, true, switch_db, db)
        end
    end
    return result
end
...
--返回:无
function _M.del(keys, switch_db, db)
    if keys and type(keys) == "table" and next(keys) and common_util.is_array(keys) then
        if switch_db and type(switch_db) == "boolean" then
            ...
            switch_db_exec_command(key, pipeline_commands, false, switch_db, db)
        end
    end
end
...
--返回:无返回
function _M.mset(mset_command, switch_db, db)
    if mset_command and type(mset_command) == "table" and next(mset_command) and common_util.is_array(mset_command) then
        ...
        switch_db_exec_command(nil, pipeline_commands, false, switch_db, db)
    end
end
...
--返回:查询结果（数组）
function _M.mget(keys, switch_db, db)
    local result = nil
    ...
    result = switch_db_exec_command(nil, pipeline_commands, true, switch_db, db)
    return result
end
...
--返回:无
function _M.hset(hset_command, switch_db, db)
    if hset_command and type(hset_command) == "table" and next(hset_command) and not common_util.is_array(hset_command) and hset_command.key and hset_command.filed and hset_command.value then
        if switch_db and type(switch_db) == "boolean" then
            ...
            switch_db_exec_command(hset_command.key, pipeline_commands, false, switch_db, db)
        end
    end
end
...
--返回:查询结果（字符串）
function _M.hget(hget_command, switch_db, db)
    local result = nil
    ...
    if switch_db and type(switch_db) == "boolean" then
        ...
        result = switch_db_exec_command(hget_command.key, pipeline_commands, true, switch_db, db)
    end
    return result
end
...
--返回:无
function _M.hmset(key, hmset_command, switch_db, db)
    if not key or type(key) ~= "string" then
        return
    end
...
--返回:查询结果（table）
function _M.hmget(key, fileds, switch_db, db)
    local result = nil
    if not key or type(key) ~= "string" then
        return result
    end
...
--返回:无
function _M.hdel(key, fileds, switch_db, db)
    if not key or type(key) ~= "string" then
        return
    end
    if not fileds or type(fileds) ~= "table" or not next(fileds) or not common_util.is_array(fileds) then
...
--返回:无
function _M.lpush(key, values, switch_db, db)
    if not key or type(key) ~= "string" then
        return
    end
...
--返回:查询结果（table）
function _M.lrange(key, start, en, switch_db, db)
    local result = nil
...
--返回:查询结果（字符串）
function _M.lindex(key, index, switch_db, db)
    local result = nil
...
--入参:key为llen的key
--返回:查询结果（数字）
function _M.llen(key, switch_db, db)
    local result = nil
    if not key or type(key) ~= "string" then
        return result
    end
...
--返回:无
function _M.lrem(key, count, value, switch_db, db)
...
--返回:无
function _M.sadd(key, values, switch_db, db)
    if not key or type(key) ~= "string" then
        return
    end
    if not values or type(values) ~= "table" or not next(values) or not common_util.is_array(values) then
...
--入参:key为smembers的key
--返回:查询结果（table）
function _M.smembers(key, switch_db, db)
    local result = nil
    if not key or type(key) ~= "string" then
        return result
    end
...
--入参:key为scard的key
--返回:查询结果（数字）
function _M.scard(key, switch_db, db)
    local result = nil
    if not key or type(key) ~= "string" then
        return result
    end
...
local function internal_set(redis_command, key, keys, switch_db, db)
    local result = nil
    if not key or type(key) ~= "string" then
        return result
    end
...
--返回:查询结果（table）
function _M.sdiff(key, keys, switch_db, db)
    return internal_set("sdiff", key, keys, switch_db, db)
end

--sinter指令
--入参:key为要获取交集的set集合的key，keys为对比哪些set集合，keys为数组
--返回:查询结果（table）
function _M.sinter(key, keys, switch_db, db)
    return internal_set("sinter", key, keys, switch_db, db)
end

--sunion指令
--入参:key为要获取并集的set集合的key，keys为对比哪些set集合，keys为数组
--返回:查询结果（table）
function _M.sunion(key, keys, switch_db, db)
    return internal_set("sunion", key, keys, switch_db, db)
end

--sismember指令
--入参:key指定的set集合，value为要检测是否存在的元素
--返回:查询结果（布尔）
function _M.sismember(key, value, switch_db, db)
    local result = nil
...
local function internal_random(redis_command, key, count, count_is_negative, switch_db, db)
    local result = nil
    if not key or type(key) ~= "string" then
        return result
    end
...
--返回:查询结果（table）
function _M.srandmember(key, count, switch_db, db)
    return internal_random("srandmember", key, count, true, switch_db, db)
end
...
--返回:查询结果（table）
function _M.spop(key, count, switch_db, db)
    return internal_random("spop", key, count, false, switch_db, db)
end

--srem指令
--入参:key指定的set集合，values为要及删除的值，values为table数组
--返回:无
function _M.srem(key, values, switch_db, db)
    if not key or type(key) ~= "string" then
        return
    end
    if not values or type(values) ~= "table" or not next(values) or not common_util.is_array(values) then
...
--返回:无
function _M.zadd(key, zset_command, switch_db, db)
    if not key or type(key) ~= "string" then
        return
    end
...
--入参:key为指定的zset集合
--返回:查询结果（数字）
function _M.zcard(key, switch_db, db)
    local result = nil
    if not key or type(key) ~= "string" then
        return result
    end
...
local function internal_range(redis_command, key, start, stop, switch_db, db)
    local result = nil
...
--返回:查询结果（table）
function _M.zrange(key, start, stop, switch_db, db)
    return internal_range("zrange", key, start, stop, switch_db, db)
end

--zrevrange指令
--入参:key为指定的zset集合，start为开始下标参数，stop为结束下标参数，start和stop都为数字
--返回:查询结果（table）
function _M.zrevrange(key, start, stop, switch_db, db)
    return internal_range("zrevrange", key, start, stop, switch_db, db)
end
...
--返回:查询结果（table）
function _M.zrangebyscore(key, min, max, switch_db, db)
    local result = nil
...
--返回:无
function _M.zrem(key, values, switch_db, db)
