...
local function db()
    local result = redis_util.select_db("s")
    ngx.say(result)
end
db()
...
local function set()
    ...
    redis_util.set(set_command, true, 2)
end
set()

local function get()
    local result_db = redis_util.get("set_test_key", true, 2)
    ngx.say(result_db)
end
get()
...
local function del()
    ...
    redis_util.del(del_command, true, 2)
end
del()
...
local function mset()
    ...
    redis_util.mset(mset_command, true, 2)
end
mset()

local function mget()
    local mget_command = {"mset_test_key_1", "mset_test_key_2", "mset_test_key_3"}
    local result = redis_util.mget(mget_command, true, 2)
...
local function hset()
    ...
    redis_util.hset(hset_command, true, 2)
end
hset()
...
local function hget()
    ...
    ngx.say(redis_util.hget(hget_command, true, 2))
end
hget()
...
local function hmset()
    ...
    redis_util.hmset("hmset_test", hmset_command, true, 2)
end
hmset()

local function hmget()
    local hmget_command = {"hmset_test_filed_1", "hmset_test_filed_2", "hmset_test_filed_3"}
    local result_db = redis_util.hmget("hmset_test", hmget_command, true, 2)
...
local function hdel()
    ...
    redis_util.hdel("hmset_test", hdel_command, true, 2)
end
hdel()

local function lpush()
    local lpush_command = {"lpush_test_value_1", "lpush_test_value_2", "lpush_test_value_3"}
    redis_util.lpush("lpush_test", lpush_command, true, 2)
end
lpush()

local function lrange()
    local result_2 = redis_util.lrange("lpush_test", 0, -1, true, 2)
...
local function lindex()
    ngx.say(redis_util.lindex("lpush_test", 0, true, 2))
end
lindex()

local function llen()
    ngx.say(redis_util.llen("lpush_test", true, 2))
end
llen()

local function lrem()
    redis_util.lrem("lpush_test", 0, "lpush_test_value_1", true, 2)
end
lrem()
...
local function sadd()
    ...
    redis_util.sadd("sadd_test_key", sadd_command, true, 2)
    redis_util.sadd("sadd_test_key_2", sadd_command_2, true, 2)
end
sadd()

local function smembers()
    local result_2 = redis_util.smembers("sadd_test_key_2", true, 2)
    for i, v in ipairs(result_2) do
        ngx.say(v)
    end
end
...
local function scard()
    ngx.say(redis_util.scard("sadd_test_key_2", true, 2))
end
scard()

local function sdiff()
    local target_sdiff_keys = {"sadd_test_key"}
    local result_2 = redis_util.sdiff("sadd_test_key_2", target_sdiff_keys, true , 2)
    for i, v in ipairs(result_2) do
        ngx.say(v)
    end
end
...
local function sinter()
    ...
    local result_2 = redis_util.sinter("sadd_test_key_2", target_sinter_keys, true, 2)
    for i, v in ipairs(result_2) do
        ngx.say(v)
    end
end
...
local function sunion()
    ...
    local result_2 = redis_util.sunion("sadd_test_key_2", target_sunion_keys, true, 2)
    for i, v in ipairs(result_2) do
        ngx.say(v)
    end
end
...
local function sismember()
    ngx.say(redis_util.sismember("sadd_test_key", "sadd_test_value_1", true, 2))
    ngx.say(redis_util.sismember("sadd_test_key", "no_value", true, 2))
end
sismember()

local function srandmember()
    local result_2 = redis_util.srandmember("sadd_test_key", 1, true, 2)
    for i, v in ipairs(result_2) do
        ngx.say(v)
    end
end
...
local function spop()
    local result_2 = redis_util.spop("sadd_test_key", 1, true, 2)
    for i, v in ipairs(result_2) do
        ngx.say(v)
    end
end
...
local function srem()
    ...
    redis_util.srem("sadd_test_key_2", srem_command, true, 2)
end
srem()
...
local function zadd()
    ...
    redis_util.zadd("zadd_test_key", zadd_command, true , 2)
end
zadd()

local function zcard()
    ngx.say(redis_util.zcard("zadd_test_key", true, 2))
end
zcard()

local function zrange()
    local result_2 = redis_util.zrange("zadd_test_key", 0, -1, true, 2)
    for i, v in ipairs(result_2) do
        ngx.say(v)
    end
end
...
local function zrevrange()
    local result_2 = redis_util.zrevrange("zadd_test_key", 0, -1, true, 2)
    for i, v in ipairs(result_2) do
        ngx.say(v)
    end
end
...
local function zrangebyscore()
    local result_2 = redis_util.zrangebyscore("zadd_test_key", "0", "2", true, 2)
    for i, v in ipairs(result_2) do
        ngx.say(v)
    end
end
...
local function zrem()
    ...
    redis_util.zrem("zadd_test_key", zrem_command, true, 2)
end
zrem()